\hypertarget{nms_8cpp}{}\section{/home/kairat/\+Dropbox/\+Computer\+Vision/\+Codes/\+Parts\+Based\+Detector/src/nms.cpp File Reference}
\label{nms_8cpp}\index{/home/kairat/\+Dropbox/\+Computer\+Vision/\+Codes/\+Parts\+Based\+Detector/src/nms.\+cpp@{/home/kairat/\+Dropbox/\+Computer\+Vision/\+Codes/\+Parts\+Based\+Detector/src/nms.\+cpp}}
...
non\+Maxima\+Suppression produces a mask (dst) such that every non-\/zero value of the mask corresponds to a local maxima of src. The criteria for local maxima is as follows\+: \begin{DoxyVerb}For every possible (sz x sz) region within src, an element is a
...
Example\+: 
\begin{DoxyCode}
1 // create a random test image
2 Mat random(Size(2000,2000), DataType<float>::type);
3 randn(random, 1, 1);
4 
5 // only look for local maxima above the value of 1
6 Mat mask = (random > 1);
7 
8 // find the local maxima with a window of 50
9 Mat maxima;
10 nonMaximaSuppression(random, 50, maxima, mask);
11 
12 // optionally set all non-maxima to zero
13 random.setTo(0, maxima == 0);
\end{DoxyCode}
